...
{   
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            foreach ($items as $item) {
                ...
                if ($item->product) {
                    // Agregar información de stock al item
                    $item->has_enough_stock = $item->product->stock >= $item->quantity;
                    $item->current_stock = $item->product->stock;
                } else {
                    // Si el producto no existe, marcar como sin stock
                    $item->has_enough_stock = false;
                    $item->current_stock = 0;
                }
                
                $itemsWithStockInfo[] = $item;
            }
    ...
    public function add(Request $request)
    {
        ...
        try {
            ...
            if ($product->stock <= 0) {
                \Log::warning('Intento de agregar producto sin stock', [
            ...
            if ($cartItem) {
                ...
                // Verificar si hay stock suficiente para la cantidad total
                if ($newQuantity > $product->stock) {
                    \Log::warning('Intento de agregar más productos que el stock disponible', [
                        'product_id' => $product->id,
                        'stock_actual' => $product->stock,
                        'cantidad_solicitada' => $newQuantity
                    ]);
                    
                    return response()->json([
                        'success' => false,
                        'message' => "No hay suficiente stock. Disponible: {$product->stock}, Solicitado: {$newQuantity}"
                    ], 422);
                }
                ...
                if ($request->quantity > $product->stock) {
                    ...
                    return response()->json([
                        'success' => false,
                        'message' => "No hay suficiente stock. Disponible: {$product->stock}, Solicitado: {$request->quantity}"
                    ], 422);
                }
    ...
    public function update(Request $request)
    {
        try {
            ...
            // Verificar si hay stock suficiente
            if ($request->quantity > $product->stock) {
                \Log::warning('Intento de actualizar a más productos que el stock disponible', [
                    'product_id' => $product->id,
                    'stock_actual' => $product->stock,
                    'cantidad_solicitada' => $request->quantity
                ]);
    ...
    public function markAsCompleted(Request $request)
    {
        try {
            ...
            \Log::error('Error en CartController@markAsCompleted: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Error al marcar el carrito como completado: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function clear(Request $request)
    {
        try {
            ...
            \Log::error('Error en CartController@clear: ' . $e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ]);
