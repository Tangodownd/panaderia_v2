          ...
          <div v-else>
            <div class="row">
              ...
              <div class="col-md-6">
                ...
                <div class="mb-3">
                  ...
                  <div class="form-check">
                    ...
                    <label class="form-check-label" for="payment_transfer">
                      Transferencia/Pago Movil/Zelle
                    </label>
                  </div>
                 <!-- <div class="form-check">
                ...
                <!-- NUEVO: Referencia + Comprobante si NO es efectivo -->
                <div class="mb-3" v-if="formData.payment_method !== 'cash'">
                  ...
                  <input
                    ...
                    placeholder="Ej: 123456 / ABCD1234"
                    :required="formData.payment_method !== 'cash'"
                  />
                  <small class="text-muted d-block mt-1">
                    Escribe el número de referencia del pago móvil / transferencia / Zelle.
                  </small>

                  <div class="card bg-cream p-2 mt-2">
                    <h6 class="mb-1">Datos para Transferencia / Pago Móvil</h6>
                    <ul class="list-unstyled mb-0 small">
                      <li><strong>Banco:</strong> Banco de Venezuela</li>
                      <li><strong>Tipo de cuenta:</strong> Corriente</li>
                      <li><strong>Número de cuenta:</strong> 0123 4567 8901</li>
                      <li><strong>Titular:</strong> Panadería Orquídea de Oro</li>
                      <li><strong>RIF/Cédula:</strong> J-12345678-9</li>
                      <li><strong>Pago Móvil (Número):</strong> +58 412-0000000</li>
                      <li><strong>Zelle (email):</strong> pagos@panaderia.com</li>
                    </ul>

                  </div>
                  <div v-if="validationErrors.payment_reference" class="text-danger mt-1">
...
<body>
  <div class="invoice">
    ...
    // Limpiar referencia y archivo si el usuario vuelve a "cash"
    watch(() => formData.value.payment_method, (pm) => {
      if (pm === 'cash') {
        formData.value.payment_reference = '';
        paymentFile.value = null;
        validationErrors.value.payment_reference = '';
    ...
    // NUEVO: validar referencia (mínimo 6 caracteres, alfanumérico + guiones)
    const validatePaymentReference = () => {
      if (formData.value.payment_method === 'cash') {
        validationErrors.value.payment_reference = '';
        return true;
      }
      const refv = (formData.value.payment_reference || '').trim();
      if (!refv) {
        validationErrors.value.payment_reference = 'El número de referencia es requerido.';
        return false;
      }
      if (refv.length < 6) {
        validationErrors.value.payment_reference = 'La referencia debe tener al menos 6 caracteres.';
        return false;
      }
      // Acepta letras, números, guiones y espacios
      if (!/^[A-Za-z0-9\- ]+$/.test(refv)) {
        validationErrors.value.payment_reference = 'Usa solo letras, números o guiones.';
        return false;
      }
      validationErrors.value.payment_reference = '';
      return true;
    ...
    const validateForm = () => {
      ...
      // NUEVO: si no es efectivo, exigir referencia y archivo
      if (formData.value.payment_method !== 'cash') {
    ...
    const submitOrder = async () => {
      ...
      try {
        ...
        const response = await axios.post('/api/orders', {
          ...formData.value,
          payment_reference: formData.value.payment_reference || null, // NUEVO
        ...
        if (formData.value.payment_method !== 'cash') {
          ...
          // NUEVO: mandar referencia también al endpoint de comprobante
          if (formData.value.payment_reference) {
            fd.append('payment_reference', formData.value.payment_reference);
          }
...
.custom-modal-backdrop {
  ...
  align-items: center;
  z-index: 1065; /* <-- antes 1050. Mayor que .modal-backdrop (1050) y .modal (1055) */
}
...
.custom-modal-content {
  ...
  z-index: 1066; /* asegúrate que el contenido esté sobre el backdrop */
}
